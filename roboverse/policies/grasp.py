...
class Grasp:
    ...
    def __init__(self, env, pick_height_thresh=-0.23, xyz_action_scale=7.0,
                 pick_point_noise=0.00, pick_point_z=-0.31, mult_reset=False, num_reset=5, single_target=True):
        self.env = env
    ...
    def reset(self):
        ...
        self.object_to_target = self.env.target_object if self.single_target else self.object_to_target
        self.pick_point = bullet.get_object_position(self.env.objects[self.object_to_target])[0]
    ...
    def get_action(self):
        ...
        if self.mult_reset and self.steps_reset >0:
            # go to neutral pos (steps_reset steps max)
