...
class Widow250Env(gym.Env, Serializable):
    ...
    def __init__(self,
                 ...
                 rand_target=False,
                 load_single=True,
                 num_obj_sample=np.inf):
        self.num_obj_sample = max(0,min(num_obj_sample, len(object_names)))
        ...
        self.num_objects = 1 if self.load_single else len(object_names)
    ...
    def reset(self):
        if self.rand_target:
            self.target_object = np.random.choice(self.object_names[:self.num_obj_sample])
            print('target', self.target_object)
            print('num_sample', self.num_obj_sample)
        bullet.reset()
    ...
    def get_info(self):
        ...
        if self.load_single:
            info['grasp_success'] = info['grasp_success_target']
        return info
